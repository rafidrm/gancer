...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        w, h = AB.size
        w2 = int(w / 2)  # width of a single img (not whole AB)
        A = AB.crop((0, 0, w2, h)).resize(
            (self.opt.loadSize, self.opt.loadSize), Image.BICUBIC)
        B = AB.crop((w2, 0, w, h)).resize(
            (self.opt.loadSize, self.opt.loadSize), Image.BICUBIC)
        # tensorize here. Could possibly just feed tensors.
        A = transforms.ToTensor()(A)
        B = transforms.ToTensor()(B)
        # after loading, I scaled. now to crop to fineSize
        w_offset = random.randint(
            0, max(0, self.opt.loadSize - self.opt.fineSize - 1))
        h_offset = random.randint(
            0, max(0, self.opt.loadSize - self.opt.fineSize - 1))
        A = A[:, h_offset:h_offset + self.opt.fineSize, w_offset:
              w_offset + self.opt.fineSize]
        B = B[:, h_offset:h_offset + self.opt.fineSize, w_offset:
              w_offset + self.opt.fineSize]
